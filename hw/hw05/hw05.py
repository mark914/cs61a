...
class VendingMachine:
    ...
    def restock(self, num):
        self.curr_stock = self.curr_stock + num
        return 'Current {0} stock: {1}'.format(self.name, self.curr_stock)
    ...
    def vend(self):
        if self.curr_stock==0:
            ...
            if self.balance < self.price:
                ...
                self.balance = 0
                self.curr_stock = self.curr_stock-1
...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.current_year-self.year-50>0:
            add_worth = Mint.current_year-self.year-50          
...
def store_digits(n):
    ...
    def store_digits_helper(lst):
        first = lst[-1]
        if len(lst) == 1:
            return Link(lst[0])
        else:
            rest = store_digits_helper(lst[:-1])
            return Link(first, rest)

    return store_digits_helper(lst)
