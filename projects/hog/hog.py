...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while times < num_rolls:
        temp = dice()
        dices.append(temp)
        total, times= total + temp, times+1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    """
    COMPONENTS
    OTHER FUNCTION: who = other(who)
    STRATEGY FUNCTION: play0_num_dice = strategy0(score0, score1)
    EXTRA_TURN FUCNTION: <boolen> = extra_turn(<score>, <score>)
    TAKE_TURN FUNCTION: score_gain = take_turn(num_dice, <score>, dice)
    """
    ...
    while True:
        if who == 0: 
            ...
            score_gain = take_turn(play0_num_dice, score1, dice)
            ...
            score_gain = take_turn(play1_num_dice, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            if int(score0 - last_score) > running_high:
            ...
            if int(score1 - last_score) > running_high:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while k <= 10:
        ...
        if aver(k, dice) > max_score:
            max_score = aver(k, dice)
            max_roll_num = k
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score + take_turn(0, opponent_score), opponent_score):
        num_rolls = 0
    elif take_turn(0,opponent_score) >= cutoff:
        num_rolls = 0
